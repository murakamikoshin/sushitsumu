...
| `preview-landscape.mp4` | 横向きの紹介動画（1920x1080・18秒・H.264） |
| `preview-portrait.mp4` | 縦向きの紹介動画（1080x1620・18秒・H.264） |

動画は実際の遊技を撮ったもの。連鎖・注文・のれん倍率が写る 18 秒を
切り出してある（上限は 20 秒）。音は入っていない。

表紙と動画は実際の画面から起こしたもの。もっと良いものが作れるなら
差し替えてよい。
...
遊んで桶を撮り、ネタの積まれた部分だけを切り出して組み直す。
文言や寸法は `cover.specs.json`、組み方そのものは `cover.tpl.html`。

**左上には触れないこと。** CrazyGames は表紙の左上に「NEW」などの札を
重ねるので、題字も桶もそこには置いていない（上は飾りの暖簾で埋めてある）。
3枚とも同じ組み方にしてあるのは、揃っている方が良いと言われているため。
